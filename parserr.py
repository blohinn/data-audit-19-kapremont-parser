...
def parse_data_for_house(reformagkh_id):
    ...
    if not location_lists_inside_tab:
        return None
...
def get_features(jobs):
    _ = {
        'positive': {
            'total_price_lt_plan_price': None,
            'exists': False,
        },
        'negative': {
            'total_price_gte_plan_price': None,
            'performer_info_not_available': None,
            'exist': False,
        },
    }
    ...
    for job in jobs:
        if job['price']['total'] and job['price']['plan']:
            if job['price']['total'] < job['price']['plan']:
                _['positive']['total_price_lt_plan_price'] = True
                _['positive']['exist'] = True
                break

    for job in jobs:
        if job['price']['total'] and job['price']['plan']:
            if job['price']['total'] >= job['price']['plan']:
                _['negative']['total_price_gte_plan_price'] = True
                _['negative']['exist'] = True
                break

    for job in jobs:
        if not job['performer']['name'] or not job['performer']['inn']:
            _['negative']['performer_info_not_available'] = True
            _['negative']['exist'] = True
            break
...
for i, house_data in enumerate(data):
    ...
    _ = {
        ...
        'jobs': jobs,
        'features': get_features(jobs),
    }
    results.append(_)
    total_file.write(json.dumps(_) + '\n')
